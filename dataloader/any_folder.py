...
def load_imgs(image_dir, num_img_to_load, start, end, skip, load_sorted, load_img):
    ...
    img_names = [name for name in img_names if name.endswith(('.png', '.jpg', '.jpeg'))]
    ...
    if num_img_to_load > len(img_names):
        print(f'Asked for {num_img_to_load:6d} images but only {len(img_names):6d} available. Exit.')
        exit()
    elif num_img_to_load == -1:
        print(f'Loading all available {len(img_names):6d} images')
    else:
        print(f'Loading {num_img_to_load:6d} images out of {len(img_names):6d} images.')
    ...
    if load_img:
        for p in tqdm(img_paths):
            img = imageio.imread(p)  # (H, W, 3) np.uint8
            #img = imageio.imread(p)[:, :, :3]  # (H, W, 3) np.uint8
            #img = np.rot90(image, k=-1)  # k=1 pour -90°
            #img = cv2.flip(img, 1)
            # Ensure 3 channels even if the image is grayscale
            if img.ndim == 2:  # Grayscale image
                img = np.stack([img] * 3, axis=-1)  # Convert to (H, W, 3)
            img_list.append(img)
        img_list = np.stack(img_list)  # (N, H, W, 3)
        img_list = torch.from_numpy(img_list).float() / 65535 # (N, H, W, 3) torch.float32
