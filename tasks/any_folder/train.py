...
def parse_args():
    ...
    parser.add_argument('--scene_name', type=str, default='inbreast')
    ...
    parser.add_argument('--learn_R', default=False, type=eval, choices=[True, False])
    parser.add_argument('--learn_t', default=False, type=eval, choices=[True, False])
...
mask_mlo = cv2.imread('MLO_mask.png', cv2.IMREAD_UNCHANGED)
...
def volume_mlo_lesion(sample_pos, rgb_density, output_path='sample_points_mlo_n_lesion.ply'):
    """
    Visualize the 3D points with their associated RGB color and save them in .ply format without flattening the arrays.
    ...
    with torch.no_grad():
        ...
        all_points_lesion = []
        all_color_lesion = []
        red_color = np.array([0, 255, 0])  # RGB for red
        # Iterate through the grid to collect points and their corresponding RGB values
        H, W, N_sample, _ = pts.shape
        for h in range(H):
            for w in range(W):
                for n in range(N_sample):
                    if mask_mlo_fin[h, w] == 255:
                        all_points_lesion.append(pts[h , w , n])
                        all_color_lesion.append(red_color)
                        #print(all_color_lesion)
...
mask_cc = cv2.imread('CC_mask.png', cv2.IMREAD_UNCHANGED)
...
def volume_cc_lesion(sample_pos, rgb_density, output_path='sample_points_cc_n_lesion.ply'):
    """
    Visualize the 3D points with their associated RGB color and save them in .ply format without flattening the arrays.
    ...
    with torch.no_grad():
        ...
        all_points_lesion = []
        all_color_lesion = []
        red_color = np.array([255, 0, 0])  # RGB for red
        # Iterate through the grid to collect points and their corresponding RGB values
        H, W, N_sample, _ = pts.shape
        for h in range(H):
            for w in range(W):
                for n in range(N_sample):
                    if mask_cc_fin[h, w] == 255:
                        all_points_lesion.append(pts[h , w , n])
                        all_color_lesion.append(red_color)
...
mask_z_2 = cv2.imread('mask_mlo_processed_1.png', cv2.IMREAD_UNCHANGED)
...
def train_one_epoch(scene_train, optimizer_nerf, optimizer_focal, optimizer_pose, model, focal_net, pose_param_net,
    ...
    for i in ids:
        ...
        #print('ray slected on a pixel :', ray_selected_cam[0][0])
        img_selected = img[:, :, :3]    # (H, W, 3), discard alpha if present
        print(img_selected.shape)
        #print('the pixel selected:' , img_selected[0][0])
        ...
        #poses_encoded = render_result['encoded_poses']
        #sample_poses_list.append(sample_poses)
        #ray_dir_list.append(ray_direction)
        #ray_ori_list.append(ray_origine)
    ...
    #images_saved[0].save('/home/yassine.ameskine/lustre/med_img-z2y8h4a967e/code_yassine/nerfstudio/nerfmm/rendered_image_cc.png')
    #images_saved[1].save('/home/yassine.ameskine/lustre/med_img-z2y8h4a967e/code_yassine/nerfstudio/nerfmm/rendered_image_mlo.png')
    
    #visualize_rays([
        #(ray_ori_list[0], ray_dir_list[0], [1, 0, 0]),   # Red for CC
        #(ray_ori_list[1], ray_dir_list[1], [0, 1, 0])  # Green for MLO
        #], step=20, ray_length=0.02, save_paths=["cc_rays.ply", "mlo_rays.ply"])
    
    #volume_cc(sample_poses_list[0], rgb_density_list[0])
    #export_ray_dirs_to_ply(ray_dir_list[1], filename='ray_directions_mlo.ply', scale=1)
    #export_ray_dirs_to_ply__(ray_dir_cam_list[0], filename='ray_directions_with_plane.ply', scale=2, fx= fxfy[0].detach().cpu().numpy(), fy= fxfy[1].detach().cpu().numpy())
    #volume_mlo(sample_poses_list[1], rgb_density_list[1])
    #volume_mlo_lesion(sample_poses_list[1], rgb_density_list[1])
    #volume_cc_lesion(sample_poses_list[0], rgb_density_list[0])
...
def main(args):
    ...
    gmlo_c2w = torch.tensor([[0.7, 0, 0.7,  3.5],
        [-0.7,  0,  0.7, 0],
        [ 0, -1,  0, -1.9014e-05],
        [ 0.0000e+00,  0.0000e+00,  0.0000e+00,  1.0000e+00]])
    
    cmlo_c2w = torch.tensor([[0.7, 0, 0.7,  2.5],
        [-0.7,  0,  0.7, 1],
        [ 0, -1,  0, -1.9014e-05],
        [ 0.0000e+00,  0.0000e+00,  0.0000e+00,  1.0000e+00]])
    ...
    cmlo_c2w = torch.tensor([[0.8, 0, 0.8,  2.9],
        [-0.8,  0,  0.8, 1],
        [ 0, -1,  0, -1.9014e-05],
        [ 0.0000e+00,  0.0000e+00,  0.0000e+00,  1.0000e+00]])
    ...
    pose_param_net = LearnPose(scene_train.N_imgs, learn_R=False, learn_t=False, init_c2w=init_c2w)
    #pose_param_net = LearnPose(scene_train.N_imgs, learn_R=True, learn_t=True, init_c2w=None)
    eval_c2ws = init_c2w  # (1, 4, 4)
    ...
    for epoch_i in tqdm(range(args.epoch), desc='epochs'):
        ...
        if epoch_i % args.eval_interval == 0 and epoch_i > 0:
            with torch.no_grad():
                eval_one_epoch(eval_c2ws, scene_train, model, focal_net, pose_param_net, my_devices, args, rgb_act_fn)
