...
def gget_ndc_rays_fxfy(H, W, fxfy, near, rays_o, rays_d):
    """
    This function is modified from https://github.com/kwea123/nerf_pl.
    ...
    d0 = -1. / (W / (2. * fxfy)) * (rays_d[..., 0] / rays_d[..., 2] - ox_oz)
    d1 = -1. / (H / (2. * fxfy)) * (rays_d[..., 1] / rays_d[..., 2] - oy_oz)
    d2 = 1 - o2

    rays_o = torch.stack([o0, o2, -o1], -1)  # (B, 3)
    rays_d = torch.stack([d0, d2, -d1], -1)  # (B, 3)
...
def get_ndc_rays_fxfy(rays_o, pts_3d=None, eps=1):
    """
    Normalize ray origins (to [-1,1]^3) and directions (unit vectors).
    
    Args:
        rays_o: (N_rays, 3) ray origins
        rays_d: (N_rays, 3) ray directions
        pts_3d: (N_points, 3), optional scene points to normalize
        eps: small constant to avoid divide by zero

    Returns:
        rays_o_normalized: (N_rays, 3)
        rays_d_normalized: (N_rays, 3)
        pts_3d_normalized: (N_points, 3) or None
        center: (3,)
        scale: float
    """
    if pts_3d is not None:
        all_xyz = torch.cat([rays_o, pts_3d], dim=0)
    else:
        all_xyz = rays_o

    # Compute scene center and scale
    min_xyz = all_xyz.min(dim=0).values
    max_xyz = all_xyz.max(dim=0).values
    center = (min_xyz + max_xyz) / 2.0
    scale = (max_xyz - min_xyz).max() / 2.0 + eps

    # Normalize origins
    rays_o_normalized = (rays_o - center) / scale


    # Normalize scene points if provided
    pts_3d_normalized = None
    if pts_3d is not None:
        pts_3d_normalized = (pts_3d - center) / scale

    return rays_o_normalized, pts_3d_normalized, center, scale
...
def volume_sampling_ndc(c2w, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t):
    ...
    if isinstance(focal, float):
        ...
        ray_ori_world , _, _ , _ = get_ndc_rays_fxfy(rays_o=ray_ori_world, pts_3d=None, eps=1e-6)  # (H*W, 3)
    ...
    sample_pos, _ = sample_query_points(ray_ori_world, ray_dir_world, near, far, N_samples=2, rand=True)

    return sample_pos, ray_ori_world, ray_dir_world, t_vals_noisy  # (H, W, N_sample, 3), (3, ), (H, W, 3), (H, W, N_sam)

def volume_sampling_ndc_before(c2w, ray_orig_cam, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t):
    """
    :param c2w:             (3/4, 4)                camera pose
    :param ray_dir_cam:     (H, W, 3)               ray directions in the camera coordinate
    :param focal:           a float or a (2,) torch tensor for focal.
    ...
    ray_dir_world = ray_dir_world.reshape(-1, 3)  # (H, W, 3) -> (H*W, 3)
    ray_ori_world = ray_ori_world.reshape(-1, 3)  # (H, W, 3) -> (H*W, 3)
    #ray_ori_world = ray_ori_world.view(1, 3).expand_as(ray_dir_world)  # (3, ) -> (1, 3) -> (H*W, 3)
    #if isinstance(focal, float):
        #ray_ori_world, ray_dir_world = get_ndc_rays(H, W, focal, 1.0, rays_o=ray_ori_world, rays_d=ray_dir_world)  # (H*W, 3)
    #else:  # if focal is a tensor contains fxfy
    ...
    ray_ori_world = ray_ori_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)
    # this perturb only works if we sample depth linearly, not the disparity.
    #ray_ori_world , ray_dir_world = prepare_ndc(H, W, focal, 0.5, ray_ori_world , ray_dir_world)
    if perturb_t:
        # add some noise to each each z_val
        t_noise = torch.rand((ray_H, ray_W, N_sam), device=c2w.device, dtype=torch.float32)  # (H, W, N_sam)
        t_noise = t_noise * (far - near) / N_sam
...
def sample_query_points(rays_o, rays_d, near, far, N_samples, rand=True):
    """
    Compute 3D query points along rays.
 
    :param rays_o: (H, W, 3) tensor of ray origins
    :param rays_d: (H, W, 3) tensor of ray directions
    :param near: float
    :param far: float
    :param N_samples: int, number of samples along each ray
    :param rand: bool, whether to use stratified sampling
    :return:
        pts: (H, W, N_samples, 3) sampled 3D points
        z_vals: (H, W, N_samples) sample depths
    """
    H, W, _ = rays_o.shape
 
    # (N_samples,) → (1, 1, N_samples)
    z_vals = torch.linspace(near, far, N_samples, device=rays_o.device).view(1, 1, N_samples)
    if rand:
        mids = 0.5 * (z_vals[..., 1:] + z_vals[..., :-1])
        upper = torch.cat([mids, z_vals[..., -1:]], -1)
        lower = torch.cat([z_vals[..., :1], mids], -1)
        t_rand = torch.rand((H, W, N_samples), device=rays_o.device)
        z_vals = lower + (upper - lower) * t_rand  # stratified sampling
 
    else:
        z_vals = z_vals.expand(H, W, N_samples)
 
    # rays_o: (H, W, 1, 3), rays_d: (H, W, 1, 3), z_vals: (H, W, N_samples, 1)
    pts = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[..., :, None]
 
    return pts, z_vals  # (H, W, N_samples, 3), (H, W, N_samples)
